        ...
        if(os.path.isdir(path)):
            ...
            for h_c in range(0,h_count):
                for w_c in range(0,w_count):
                    h = h_c * sample_step
                    w = w_c * sample_step
                    sub_s2 = img[h:h + sample_size, w:w + sample_size,:].astype(np.int)
                    sub_s1 = binfiles[h:h + sample_size, w:w + sample_size,:].astype(np.int)

                    save_img = np.concatenate((sub_s2,sub_s1), axis=1)
                    save_path = os.path.join(path, "split")
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    
                    cv.imwrite(save_path + '/' + str(city_count) +'.png', save_img)
